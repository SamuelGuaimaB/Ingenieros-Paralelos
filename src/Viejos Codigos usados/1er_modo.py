...
class WROPrimitivoBlindado:
    def __init__(self):
        ...
        # --- RANGOS HSV PARA DETECTAR SENTIDO DE PISTA ---
        # Naranja (Normalmente Hue entre 5 y 20)
        self.lower_naranja = np.array([5, 45, 45])
        self.upper_naranja = np.array([20, 255, 255])
        
        # Azul (Normalmente Hue entre 100 y 130)
        self.lower_azul = np.array([95, 50, 50])
        self.upper_azul = np.array([135, 255, 255])
    ...
    def process_vision(self):
        ...
        while self.running:
            ...
            roi_pista = blur[30:140, :]
            _, binarizada = cv2.threshold(
                roi_pista, 88, 255, cv2.THRESH_BINARY)
            ...
            if self.SENTIDO_GIRO == "AUTO" and self.estado_general == "CARRERA":
                # Franja alta para ver los bloques a lo lejos
                roi_colores = frame[90:220, :] 
                hsv = cv2.cvtColor(roi_colores, cv2.COLOR_BGR2HSV)

                mask_naranja = cv2.inRange(hsv, self.lower_naranja, self.upper_naranja)
                mask_azul = cv2.inRange(hsv, self.lower_azul, self.upper_azul)

                kernel_color = np.ones((3,3), np.uint8)
                mask_naranja = cv2.morphologyEx(mask_naranja, cv2.MORPH_OPEN, kernel_color)
                mask_azul = cv2.morphologyEx(mask_azul, cv2.MORPH_OPEN, kernel_color)

                contornos_naranjas = cv2.countNonZero(mask_naranja)#, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                contornos_azules = cv2.countNonZero(mask_azul)#, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                
                umbral_linea = 120

                if contornos_naranjas > umbral_linea or contornos_azules > umbral_linea:
                    if contornos_naranjas > contornos_azules:
                        self.SENTIDO_GIRO = "DERECHA"
                        print(">>> BLOQUE NARANJA DETECTADO: Pista a la DERECHA <<<")
                    else:
                        self.SENTIDO_GIRO = "IZQUIERDA"
                        print(">>> BLOQUE AZUL DETECTADO: Pista a la IZQUIERDA <<<")
            ...
            if es_muro_frontal:
                ...
                if not self.en_curva and (current_time - self.tiempo_ultima_curva > 3.2):
                    ...
                    if self.curvas_superadas % 4 == 0:
                        ...
                        print(
                            f"\n[OK] VUELTA {self.curvas_superadas}/12 COMPLETADA \n")

                    if self.curvas_superadas == 12:
                ...
                if current_time - self.tiempo_ultima_curva > 4.2: #2 #2.8
                    self.en_curva = False
            ...
            if self.VER_PANTALLAS:
                ...
                cv2.imshow("Vision Procesada (Piso)", binarizada)
                
                # 2. Mostrar las máscaras de color (SOLO si el robot sigue en modo "AUTO")
                # Las ponemos dentro de este 'if' porque una vez que el robot decide el sentido,
                # deja de calcular los colores para ahorrar procesador, y las variables dejarían de existir
        ...
        while self.running:
            if self.estado_general == "ESPERA":
                if self.boton_presionado:
                    ...
                    # Check whether the camera already sees the black wall before startin
                ...
                if self.serial_enabled:
                    if self.SENTIDO_GIRO == "DERECHA":
                        self.ser.write(b"<200,65>\n")
                        time.sleep(0.7)
                        self.ser.write(b"<200,86>\n")
                        time.sleep(0.7)
                    elif self.SENTIDO_GIRO == "IZQUIERDA":
                        self.ser.write(b"<200,109>\n")
                        time.sleep(0.7)
                        self.ser.write(b"<200,86>\n")
                        time.sleep(0.7)
